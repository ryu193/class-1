...
select job,substr(job,0,2) as 간략히,
substr(job,3,3) as "세번째부터 세글자",
substr(job,4),
substr(job,4,100)
from emp;


select job,substr(job,4)as 간략히,
length(substr(job,4))as 길이
from emp;


select job,substr(job,-3)as 간략히,
length(substr(job,-3))as 길이
from emp;
...
select ename,substr(ename,0,2)as 간략히
from emp;

select
replace(ename,substr(ename,0,2),'**')
from emp;
--ename에서 앞에 두글자만 **로 처리
select
replace(ename,substr(ename,0,1),'**')
from emp;


--ename의 앞에 반절 모두 ** 로 처리
select
...
--ename의 뒤의 반절모두 ** 로 처리
select
replace(ename,substr(ename,-(length(ename)*0.5)),'**')
from emp;

/*따라쓰기-이해하기-이해를배경으로 다시풀어보기
-응용해서 비슷한내용을 다르게 풀어보고 해석해보기*/
...
--문제1
select '010-8385-5714',
length('010-8385-5714') as 자릿수,
substr('010-8385-5714',1,length('010-8385-5714')-4) as "잘린글씨" ,
rpad(substr('010-8385-5714',1,length('010-8385-5714')-4),length('010-8385-5714')
,'*')
from dual;
...
select ename,
length(ename) as 자릿수,
substr(ename,1,length(ename)-(length(ename)-2)) as "잘린글씨" ,
rpad(substr(ename,1,length(ename)-(length(ename)-2)),
length(ename)
,'*')
from emp;
...
select ename,
length(ename) as 자릿수,
substr(ename,-(length(ename)-1)) as "잘린글씨" ,
lpad(substr(ename,-(length(ename)-1)),
length(ename)
,'*')
from emp;
...
--심화1 job을 대상으로 20자리의 공간에서 가운데 정렬
select lpad(job,((17+length(job))/2),'*')
from emp;

   
select rpad(lpad(job,((19+length(job))/2),'*'),20,'*')
from emp;
